# Resident protocol

A resident is a bounded participant in the environment, not an autonomous world
owner. It declares:

- the evidence and domain context it can read;
- the interpretations it can emit;
- the actions it can propose;
- the capabilities it may exercise, if any;
- its uncertainty and abstention behavior; and
- the canonical domain owner to which accepted changes are reconciled.

Residents do not share a single compulsory consciousness. Human Model, Bridget,
Coach, Banjo, and future residents can remain independent implementations. A
world may route an artifact to zero, one, or several residents, and conflicting
interpretations remain attributed rather than silently merged.

This directory intentionally defines the public protocol only. Private resident
prompts, memories, policies, and lived data are outside the publication boundary.
